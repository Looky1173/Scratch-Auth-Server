...
setInterval(async () => {
    ...
    switch (latestQueue.type) {
        ...
        case queue.TYPES.CommentVerification:
            fetch(`https://api.allorigins.win/raw?url=https://api.scratch.mit.edu/users/${AUTH_PROJECT.author}/projects/${AUTH_PROJECT.id}/comments?offset=0&limit=20&cache=${Date.now()}`)
        ...
        case queue.TYPES.ProfileCommentVerification:
            const res = await fetch(`https://api.allorigins.win/raw?url=https://scratch.mit.edu/site-api/comments/user/${latestQueue.data.username}?cache=${Date.now()}`);
